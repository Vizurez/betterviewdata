...
function PANEL:Populate(player, data, pointTable, CID)
	...
	listView.DoDoubleClick = function(_, _, line)
		self:ShowFullReason(line);
	end;

	local reasonButton = vgui.Create("DButton", rightDock);
	reasonButton:SetText("View Full Reason");
	reasonButton:SetTall(25);
	reasonButton:DockMargin(0, 4, 0, 0);
	reasonButton:Dock(BOTTOM);
	reasonButton.DoClick = function()
		local selected = listView:GetSelected();

		if (next(selected) != nil) then
			self:ShowFullReason(selected[1]);
		end;
	end;
